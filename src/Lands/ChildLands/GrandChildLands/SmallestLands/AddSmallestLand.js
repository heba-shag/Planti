...
export default function AddSmallestLand(){
  ...
  const grandParentLinkId=window.location.pathname.split("/").slice(-6)[0];
  ...
  fetch(`http://cultivation.runasp.net/api/Land/GetById?id=${grandParentLinkId}`)
    .then((res)=>{
      if(!res.ok){
          throw Error("couldn't fetch data for that resource" )
      }
  ...
  return(
    <AddForm
      ...
      direction={`lands/child/${grandParentLinkId}/grandchild/${parentLinkId}/pieceLand/${id}`}
